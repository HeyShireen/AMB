...
class Settings(BaseSettings):
    ...
    stop_loss_pct: float = Field(0.12, description="Fractional stop-loss (0.12 = 12%)")
    take_profit_pct: float = Field(0.20, description="Fractional take-profit (0.20 = 20%)")
    partial_take_profit_pct: float = Field(0.10, description="Partial TP trigger (0.10 = 10%)")
    partial_take_profit_frac: float = Field(0.5, description="Fraction to sell at partial TP")
    trailing_stop_enabled: bool = Field(True, description="Enable trailing stop")
    trailing_stop_activation_pct: float = Field(0.08, description="Activate trailing after this gain")
    trailing_stop_distance_pct: float = Field(0.05, description="Trail distance from high")
    max_positions: int = Field(8, description="Maximum simultaneous holdings")
    momentum_lookback: int = Field(20, description="Days for momentum calculation")
    min_momentum: float = Field(0.02, description="Minimum momentum to buy (2%)")
    rebalance_enabled: bool = Field(True, description="Enable monthly rebalance rules")
    rebalance_winner_pct: float = Field(0.12, description="P&L threshold to trim winners")
    rebalance_winner_sell_frac: float = Field(0.4, description="Fraction of a winning position to sell when trimming")
